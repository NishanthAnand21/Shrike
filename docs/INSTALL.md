...
shrike is a single self-contained Rust binary. It has **no runtime dependencies of
...
shrike runs whatever is on your `PATH`. It degrades gracefully — a tool that isn't
installed simply fails when you launch it, it doesn't stop shrike. On Kali you already
...
Optional extras shrike knows about: `rustscan`, `masscan`, `feroxbuster`, `ffuf`,
...
> shrike never bundles or downloads these — it only constructs and runs commands for
...
```bash
git clone <your-remote>/shrike.git      # or cd into the existing checkout
cd shrike
cargo build --release
```

The binary lands at `target/release/shrike` (~3.5 MB, no shared-lib surprises).
...
```bash
# from the repo root
cargo install --path .          # installs to ~/.cargo/bin/shrike
# …or just symlink the built binary
sudo ln -s "$(pwd)/target/release/shrike" /usr/local/bin/shrike
```

Now `shrike` runs from anywhere.
...
```bash
shrike --name test --targets 127.0.0.1
```
...
| `nxc: command not found` | netexec was formerly `crackmapexec`; shrike also probes `netexec`/`crackmapexec` |
...
- **macOS** — shrike itself builds and runs fine; many orchestrated tools install via
...
- **Windows** — not a supported host for the TUI; run shrike from WSL2.
