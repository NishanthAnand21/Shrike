...
fn bin() -> &'static str { env!("CARGO_BIN_EXE_warden") }

#[test]
fn plan_classifies_pivot_segment() {
    let dir = std::env::temp_dir().join(format!("warden-it-{}", std::process::id()));
    ...
    let out = Command::new(bin())
        .args(["--name", "t", "--workspace", dir.to_str().unwrap(),
               "--import", xml.to_str().unwrap(),
               "--targets", "172.16.5.9", "--plan"])
        .output().unwrap();
    let s = String::from_utf8_lossy(&out.stdout);
    assert!(s.contains("192.168.1.0/24"), "missing scanned segment:\n{s}");
    assert!(s.contains("DIRECT"), "scanned segment should be direct");
    assert!(s.contains("172.16.5.0/24"), "missing pivot segment");
    assert!(s.contains("PIVOT REQUIRED"), "unscanned segment should need a pivot");
